...
import { regionalAttribution } from './source-attribution.js';

import { SPAIN_PARTY_DEFINITIONS, SPAIN_EVENT_CATEGORIES, SPAIN_POLITICAL_EVENTS, SPAIN_COMPARISON_GROUPS, isoTime, isoDate, groupValue, pollingSnapshot, buildSpainPollingInsights } from "./spain-data.js";
export { SPAIN_PARTY_DEFINITIONS, SPAIN_EVENT_CATEGORIES, SPAIN_POLITICAL_EVENTS, buildSpainPollingInsights } from "./spain-data.js";
...
function issuePageLanguage(locale, issue) {
  ...
  if (locale === "es") return {
    ...
    methodTitle: "Qué conviene saber", method: "Barómetro mensual del CIS. Entrevistas telefónicas a población española de 18 años o más. Los datos describen respuestas en el momento del trabajo de campo y no demuestran por qué cambian las opiniones.", interviews: "entrevistas", fieldwork: "Trabajo de campo", source: "Resultados del CIS", study: "Tabulación completa del estudio",
  };
  if (locale === "de") return {
    labels, back: "Zurück zu Spanien", eyebrow: "CIS-Barometer", title: "Was Spanien beschäftigt", intro: "Ein kompakter Blick darauf, was die Bevölkerung als Problem des Landes nennt, was sie persönlich betrifft und wie sie die Wirtschaft einschätzt.",
    ...
    methodTitle: "Was man wissen sollte", method: "Monatliches CIS-Barometer. Telefoninterviews mit spanischen Staatsangehörigen ab 18 Jahren. Die Daten beschreiben Antworten zum Befragungszeitpunkt und erklären nicht, warum sich Meinungen verändern.", interviews: "Interviews", fieldwork: "Feldzeit", source: "CIS-Ergebnisse", study: "Vollständige Auswertung der Studie",
  };
  return {
    labels, back: "Back to Spain", eyebrow: "CIS barometer", title: "What concerns Spain", intro: "A compact view of what people name as a national problem, what affects them personally and how they judge the economy.",
    ...
    methodTitle: "What to know", method: "Monthly CIS barometer. Telephone interviews with Spanish citizens aged 18 and over. The figures describe answers during the fieldwork period and do not establish why opinions change.", interviews: "interviews", fieldwork: "Fieldwork", source: "CIS results", study: "Full study tables",
  };
}

function ConcernBars({ id, locale, title, intro, items, labels, info, dataDate, study, sources = [] }) {
  ...
  return (
    <article ref={exportRef} id={id} className="spain-concern-panel">
      ...
      <header><div className="widget-info-heading"><MiniGraphInfo locale={locale} title={info.title} paragraphs={info.paragraphs ?? [info.text]} sources={sources} dataDate={dataDate} /><div><h2>{title}</h2><p>{intro}</p></div></div><PngExportButton elementRef={exportRef} filename={`pollframe-${id}`} title={title} subtitle="España · CIS" locale={locale} label={exportLabel} credit={`CIS · ${study} · ${dataDate} · ${sources[0]?.href ?? "https://www.cis.es/"}`} profile="issues" className="widget-share-trigger spain-concern-png" /></header>
...
export function SpainIssuesPage({ locale, summary, formatDate, numberLocale }) {
  const text = issuePageLanguage(locale, summary.issues);
  ...
  const studyContext = locale === "es"
    ? `Fuente: barómetro mensual del CIS, estudio ${issue.study}; ${issue.interviews.toLocaleString(numberLocale)} entrevistas telefónicas a población española de 18 años o más, realizadas del ${fieldwork}. La fecha mostrada es el final del trabajo de campo, no la fecha de publicación. Pollframe reproduce y ordena los porcentajes publicados; no recalcula respuestas individuales. Los valores están redondeados y esta medición es una fotografía del periodo, no una tendencia ni una explicación causal.`
    : locale === "de"
      ? `Quelle ist das monatliche CIS-Barometer, Studie ${issue.study}: ${issue.interviews.toLocaleString(numberLocale)} Telefoninterviews mit spanischen Staatsangehörigen ab 18 Jahren vom ${fieldwork}. Der angezeigte Datenstand ist das Ende der Feldzeit, nicht das Veröffentlichungsdatum. Pollframe übernimmt und sortiert die veröffentlichten Prozentwerte, berechnet aber keine Individualdaten neu. Die Werte sind gerundet; diese Erhebung ist eine Momentaufnahme, kein Trend und keine Kausalerklärung.`
      : `Source: monthly CIS barometer, study ${issue.study}; ${issue.interviews.toLocaleString(numberLocale)} telephone interviews with Spanish citizens aged 18 and over conducted ${fieldwork}. The displayed data date is the end of fieldwork, not publication. Pollframe reproduces and ranks the published percentages; it does not recalculate respondent-level data. Figures are rounded and this survey is a snapshot, not a trend or causal explanation.`;
  const sources = [
    { href: issue.sourceUrl, label: text.source },
    { href: issue.studyUrl, label: text.study },
    ...(issue.technicalUrl ? [{ href: issue.technicalUrl, label: locale === "es" ? "Ficha técnica" : locale === "de" ? "Methodenbericht" : "Technical sheet" }] : []),
  ];
  return (
    <main id="top" className="germany-country-overview spain-country-overview spain-issues-page">
      ...
      <section className="spain-concern-grid" aria-label={text.title}>
        <ConcernBars id="spain-national-concerns" locale={locale} title={text.national} intro={text.nationalIntro} items={national.slice(0, 8)} labels={text.labels} info={{ title: text.nationalInfoTitle, paragraphs: [text.nationalInfo, studyContext, locale === "es" ? "Esta vista muestra los ocho temas más citados; las tablas enlazadas contienen todas las categorías. Las categorías y su redacción proceden de la tabulación del CIS." : locale === "de" ? "Diese Ansicht zeigt die acht häufigsten Themen; alle Kategorien stehen in der verlinkten Originaltabelle. Kategorien und Bezeichnungen folgen der CIS-Auswertung." : "This view shows the eight most-cited topics; the linked source tables contain all categories. Categories and wording follow the CIS tables."] }} study={issue.study} dataDate={issue.fieldwork[1]} sources={sources} />
        <ConcernBars id="spain-personal-concerns" locale={locale} title={text.personal} intro={text.personalIntro} items={issue.personal.slice(0, 8)} labels={text.labels} info={{ title: text.personalInfoTitle, paragraphs: [text.personalInfo, studyContext, locale === "es" ? "Solo se muestran las categorías personales publicadas e incorporadas a esta edición. No deben restarse de los porcentajes nacionales porque las preguntas y el significado son distintos." : locale === "de" ? "Gezeigt werden die für diese Ausgabe veröffentlichten und übernommenen persönlichen Kategorien. Sie dürfen nicht von den nationalen Werten abgezogen werden, weil Frage und Bedeutung verschieden sind." : "Only the personal categories published and included for this edition are shown. They should not be subtracted from national percentages because the questions and meaning differ."] }} study={issue.study} dataDate={issue.fieldwork[1]} sources={sources} />
      </section>
...
export function SpainRegionPage({ locale, regions, area, formatDate }) {
  ...
  return (
    <main id="top" className="germany-country-overview spain-country-overview spain-region-page">
      ...
      <footer className="spain-region-source"><span>{text.updated}: {region.coverage.latestDate ? `${formatDate(region.coverage.latestDate, locale, { year: true })} · ${dataAgeLabel(region.coverage.latestDate, locale)}` : "—"}</span><a href={region.sourceUrl} target="_blank" rel="noreferrer">{text.source} ↗</a></footer>
      <p className="spain-region-reuse">Wikipedia contributors · <a href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA 4.0</a> · {locale === 'de' ? 'Eigene Aufbereitung und Berechnung' : locale === 'es' ? 'Preparación y cálculo propios' : 'Own processing and calculation'} · {regionalAttribution(region).sourceUrls.map((url, index) => <React.Fragment key={url}>{index > 0 && ' · '}<a href={url} target="_blank" rel="noreferrer">{locale === 'de' ? 'Quellenarchiv' : locale === 'es' ? 'Archivo de origen' : 'Source archive'} {index + 1}</a></React.Fragment>)} · <a href="/data/source-receipt.json">{locale === 'de' ? 'Quellenverzeichnis' : locale === 'es' ? 'Registro de fuentes' : 'Source receipt'}</a></p>
    </main>
  );
}
